...
def main():
    ...
    wandb.login()
    wandb.init(project="huggingface", name="retrainwithtagsv2ibs_indicw2v_ad0_3_hd_02_featd_0_3_lr6e-4_warmup500_s300_shuff100", group="training with cs tags v2")
    ...
    # the tokenizer
    # load config
    #model_args.cache_dir= "/scratch/elec/puhe/p/palp3/MUCS/mucs_language_segregated_data/trainwithtags_warmup500_s300_shuff100/output"
    ...
    tokenizer = Wav2Vec2CTCTokenizer("/scratch/elec/puhe/p/palp3/MUCS/mucs_language_segregated_data/vocab3_MUCS.json", 
                                     unk_token=unk_token, 
                                     pad_token=pad_token, 
...
#processor = AutoProcessor.from_pretrained("ai4bharat/indicwav2vec-hindi")
    ...
    with training_args.main_process_first(desc="dataset map preprocessing"):
        vectorized_datasets = raw_datasets.map(
            ...
            cache_file_names={"train":"/scratch/elec/puhe/p/palp3/MUCS/mucs_language_segregated_data/train_datasetwtagsv2_cache_20000samples_seed300shufseed100.arrow", "eval":"/scratch/elec/puhe/p/palp3/MUCS/mucs_language_segregated_data/test_datasetwtagsv2_cache.arrow"},
    ...
    def compute_metrics(pred, output_file="/scratch/elec/puhe/p/palp3/MUCS/mucs_language_segregated_data/trainwithtagsv2ibs_gradaccu1fp16false_warmup500_s300_shuff100/predictionswtagsv2ibs_gradaccu1_fp16true_indicw2v_ad0_3_hd_02_featd_0_2_lr6e-4_warmup500_s300_shuff100.txt"):
        ...
        #print("\n")
        #print(pred_logits)
        print("predictions output file at:",output_file)
        ...
        #pred_str = [remove_ignored_chars(p, chars_to_ignore) for p in pred_str]
        #label_str = [remove_ignored_chars(l, chars_to_ignore) for l in label_str]

        #pred_str = re.sub("\s*[∅∇]\s*"," ", pred_str)
        #label_str = re.sub("\s*[∅∇]\s*"," ", label_str)
        pred_str_cleaned = [re.sub(r"\s*[∅∇]\s*", " ", s) for s in pred_str]
        label_str_cleaned = [re.sub(r"\s*[∅∇]\s*", " ", s) for s in label_str]
        print("cleaned cs transcription pred and ref \n")
        print(pred_str_cleaned[0:3])
        print(label_str_cleaned[0:3])
        metrics = {k: v.compute(predictions=pred_str_cleaned, references=label_str_cleaned) for k, v in eval_metrics.items()}
